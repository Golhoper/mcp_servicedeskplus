...
MCP (Model Context Protocol) server for integration with ServiceDesk Plus On-Premise with full CMDB support.
...
2. **Create a `.env` file with configuration details:**
```env
...
Add to the MCP configuration file (`~/.config/claude/desktop-config.json`):
...
"Create a user group 'IT Support Team' and assign permissions"
"Create an admin user 'john.doe' with role technician"
"Update permissions for the role 'manager'"
"Create a department 'Software Development'"
...
- **🔐 Authentication** - Support for Basic Auth and API Key
- **📄 Pagination** - Efficient large data management
...
1. **Centralized Management** - Centralized management of all IT infrastructure
...
7. **AI Integration** - AI integration for task automation
